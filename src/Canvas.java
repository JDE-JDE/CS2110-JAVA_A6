...
public class Canvas extends JPanel implements MouseListener, MouseMotionListener  {
    ...
    private void colorClick(Graphics2D g2d, Color c, int x, int y) {
    	...
    	g2d.setColor(c);
    	g2d.fillRect(x - toolSize/2, y - toolSize/2, toolSize, toolSize);
    	window.setImageUnsaved();
    	repaint();
    }
    
    /** Draws a line with color c and some stroke st from mouse position prev to position last*/
    private void drawLine(Color c, Graphics2D g2d, Point2D.Double prev, Point2D.Double last, Stroke st) {
    	g2d.setColor(c);
    	g2d.setStroke(st);
    	g2d.drawLine((int) prev.x, (int)prev.y, (int)last.x, (int)last.y);
    }
    ...
    private void colorDrag(Graphics2D g2d, Color c) {
    	// TODO #08 Implement me!
    	 ...
    	 Stroke bs = new BasicStroke(toolSize);
 	 drawLine(c, g2d, mousePosPrev, mousePos, bs);
 	window.setImageUnsaved();
	repaint();
     }
    ...
    private void airBrush(Graphics2D g2d, Color c) {
    	...
    	for (int i = 0; i < 160; i = i + 1) {
    		double randX= random.nextDouble()*toolSize-toolSize*.5;
    		double randY= random.nextDouble()*toolSize-toolSize*.5;
    		g2d.drawLine((int) (mousePos.x+randX), (int) (mousePos.y+randY),(int) (mousePos.x+randX), (int) (mousePos.y+randY));
    	}
    	window.setImageUnsaved();
    	repaint();
    }
    ...
    @Override public void mousePressed(MouseEvent e) {
        ...
        if (activeTool == Tool.PENCIL) {
            ...
            if (pointPressed) {
            	Stroke bs = new BasicStroke(toolSize);
            	drawLine(foreGroundColor(), g2d, firstPoint, mousePos, bs);
            	window.setImageUnsaved();
            	repaint();
            	pointPressed= false;
            } else {
